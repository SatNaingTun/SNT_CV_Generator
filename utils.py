...
def extract_text_from_file(filepath: str) -> str:
    ...
    if ext in [".tex", ".txt", ".md"]:
        ...
        if pypdf is None:
            raise ImportError("pypdf library is required for PDF parsing: pip install pypdf")
        text_content = []
...
def copy_cv_assets(cv_folder: str, output_dir: str, ignore_folders: list = None):
    ...
    for item in os.listdir(cv_folder):
        ...
        if os.path.isdir(src_path):
            ...
            shutil.copytree(src_path, dst_path, ignore=shutil.ignore_patterns(*ignore_folders, "*.pyc"))
...
def strip_straggling_pagebreaks(tex_content: str) -> str:
    ...
    If the section following \\newpage is <= 4 lines (1-3 lines), removes the break.
    """
    try:
        ...
        for pb in pagebreaks:
            ...
            # Omit page break if section following is only 1-3 lines
            if len(lines) <= 4:
                pb.delete()
                print("[+] Removed straggling \\newpage command (section <= 4 lines).")
...
def compile_latex_to_pdf(tex_filepath: str, output_dir: str) -> str:
    ...
    try:
        subprocess.run(
            ["pdflatex", f"-output-directory={output_dir}", "-interaction=nonstopmode", tex_filepath],
            check=True,
